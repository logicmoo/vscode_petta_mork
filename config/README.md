...
- [`upstreams.list`](upstreams.list) is the source of truth for every external repository cloned into `../upstreams/`. Each line follows `name url branch pin`, where the pin is usually `AUTO` or `TIP`. The automation scripts (`make bootstrap`, `make lock`, `make unlock`) read and modify this file.
