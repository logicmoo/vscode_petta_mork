...
This repo is the “one folder to open in VS Code.” It keeps the PeTTa runtime, the MORK kernel, PathMap, and the MeTTa/MM2 tooling side-by-side so the editor (and its Language Server Protocol clients) can see every dependency without juggling multiple clones. The SWI-Prolog LSP server that powers MeTTa/MM2 authoring lives under `upstreams/metta-wam/libraries/lsp_server_metta`, and VS Code picks it up through the shared `.config/swi-prolog/pack` tree created by `scripts/link_toolchains.sh`. Because PeTTa itself is mostly SWI-Prolog, we also vend and install James Cash’s `prolog_lsp` pack so VS Code gets hover/completion/diagnostics when editing Tacode or the underlying Prolog runtime. With everything wired through a single devcontainer, you can hack on PeTTa Tacode files, run the LSP-backed IDE features, and jump into upstream sources to expand or debug the stack—all without leaving this workspace.
